var SITE_URL = 'http://' + window.location.host + 'default.htm';
var STATIC_URL = SITE_URL;
var IMAGE_URL = SITE_URL;
var FILE_URL = SITE_URL;
var WWW_URL = '../www.zhaosuliao.com/';
var open_time;
$.ajaxSetup({
    "cache": false
});
var publishingSuccess = {
    ...
    addHtml: function(html) {
        ...
        that.fbBomb.css({
            'width': that.setings.width,
            'height': that.setings.height,
            'marginTop': 0,
            'top': h
        });
    },
...
function show_dialog_login() {
    ...
    art.dialog({
        ...
        title: "会员登录",
        id: "login_dialog",
        iframe: SITE_URL + "/user/login/mini?referer=" + referer
    });
}
...
function tip_open_time() {
    ...
    if (open_time <= 0) {
        return confirm('找磁材网交易时间为工作日9：00~17：30，非工作时间发布的委托采购会延迟处理，给您带来不便，请见谅。');
    }
    return true;
}
var countdown = {
...
$().ready(function() {
    ...
    if ($(".area_wrapper").length) {
        $(".area_wrapper").areaPop({
            ...
            url: {
                city: SITE_URL + "/admin/common/region",
                province: SITE_URL + "/resources/fore/plugin/jquery-areapop/jquery-areapop-province.json"
            }
        });
    }
...
var supplyCommodity = {
    ...
    addEvent: function() {
        var that = this;
        $(document).on('click', '.close_supply', function() {
            that.toggleHide();
    ...
    addHtml: function(w, h, str, info, mid, purchase_id) {
        var that = this;
        that.arr.push(['<div id="mysupply" style="display:block;">', '<div class="supply_title">', '<span class="supply_title_text">我要供货</span>', '<a class="close_supply" href="javascript:void(0)"></a>', '</div>', '<div class="supply_content">', '</div>', '</div>'].join(""));
