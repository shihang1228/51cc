...
if($(".input_div").length>0){
	...
	$(".fb_btn").on("click",function(){
        var guige_array = [];
        var goodsid_array = new Array();
        var guige_i = 0;
		$(".biaozhun").each(function(i){
			var varietyid = $(this).find(".varietyid").val();
			var gradeid = $(this).find(".gradeid").val();
			var factoryid = $(this).find(".factoryid").val();
			var quantity = $(this).find(".quantity").val();
			var unitprice = $(this).find(".unitprice").val();
			var unitid = $(this).find(".unitid").val();
			var deliverydate = $(this).find(".deliverydate").val();
			var deliveryplace = $(this).find(".deliveryplace").val();
			var warehouse = $(this).find(".warehouse").val();
			var specid = $(this).find(".specid").val();
			var claddingid = $(this).find(".claddingid").val();
			var length_diameter = $(this).find(".length_diameter").val();
			var width_aperture = $(this).find(".width_aperture").val();
			var height_thickness = $(this).find(".height_thickness").val();
			...
			guige_array[guige_i]={
				...
				'length_diameter':length_diameter,
				'width_aperture':width_aperture,
				'height_thickness':height_thickness,
				'comments':comments
			};
			guige_i = guige_i + 1;
		})
		var total_array = {guige_array:guige_array};
		$.ajax({
			url:"<?php echo U('Home/Purchase/purchaseadd_bgd');?>",
			type:"POST",
			data:total_array,
			success:function(data){
				if(data.result){
					alert("提交成功",{time: 1000},function(){
						window.location.href="<?php echo U('Home/User/member');?>";
					});
				}else if(data.result == false){
					alert(data.msg+"失败",{time: 2000});
				}
			}
		})
	})
	...
	<div class="fbcg">
		...
		<form method="post" action="<?php echo U('Home/Purchase/purchaseadd_bgd');?>" id="myform">
			...
			<div class="tab_content">
				<div class="toggle_div" style="display:none;">
					<div class="biaozhun pr" >
						<div class="input_div">
							...
							<span class="span_input">
								<select name="factoryid" class="factoryid">
							...
							<span class="span_input">
								<input type="text" name="unitprice" class="unitprice"/>
							</span>
						</div>
						<div class="input_div mar_t10 mar_b20">
							<span class="span_text">单位：</span>
							<span class="span_input">
								<select name="unitid" class="unitid">
							...
							<span class="span_input">
								<input type="text" name="deliverydate" class="deliverydate"/>
							</span>
							<span class="span_text">地点：</span>
							<span class="span_input">
								<input type="text" name="deliveryplace" class="deliveryplace"/>
							</span>
							<span class="span_text">仓库：</span>
							<span class="span_input">
								<input type="text" name="warehouse" class="warehouse"/>
							</span>
						</div>
						<div class="input_div mar_t10 mar_b20">
							<span class="span_text">形状：</span>
							<span class="span_input">
								<select name="specid" class="specid">
							...
							<span class="span_input">
								<select name="claddingid" class="claddingid">
							...
							<span class="span_input">
								<input type="text" name="length_diameter" class="length_diameter"/>
							</span>
							<span class="span_text mar_l0">宽：</span>
							<span class="span_input">
								<input type="text" name="width_aperture" class="width_aperture"/>
							</span>
							<span class="span_text mar_l0">高：</span>
							<span class="span_input">
								<input type="text" name="height_thickness" class="height_thickness"/>
							</span>
						</div>
						<div class="input_div mar_t10 mar_b20">
							<span class="span_text mar_l0">备注：</span>
							<span class="span_input">
								<input class="max_input" type="text" name="comments" class="comments"/>
							</span>
						</div>
					</div>
				    <div class="add_btn">增加一行</div>
				 </div>
				 <div class="text toggle_div" style="display:block;">
			 ...
			 <input type="button" class="fb_btn" value="委托找货" />
